...
async function batchInsert(conn, table, columns, rows) {
    ...
    const [result] = await conn.execute(`INSERT INTO \`${table}\` (${columns.join(',')}) VALUES ${placeholders}`, flat);
...
async function main() {
    ...
    try {
        ...
        await dest.execute("INSERT INTO agencies (id, name, nit, logo_url) VALUES (1, 'Construvida Agency', '000.000.000-0', NULL)");
        ...
        const [catMaps, catSizes] = await (async () => {
            ...
            for (const cat of catTables) {
                ...
                const map = new Map();
                const nameRows = rows.map(r => [r.name]);
                ...
                for (const ins of inserted) {
                    const old = rows.find(r => r.name === ins.name);
                    if (old)
                        map.set(old.id, ins.id);
                }
                maps[cat.table] = map;
        ...
        // --- COMPANIES ---
        startPing(orig);
        const [companyRows] = await orig.query('SELECT id, name, nit, address, phone, email, is_active FROM companies');
        ...
        for (const ins of insertedCompanies) {
            const old = companyRows.find((r) => r.name === ins.name && (r.nit ?? '') === ins.nit);
            if (old)
                companyMap.set(old.id, ins.id);
        }
        log(`companies: ${companyMap.size} registros migrados`);
        stopPing();
        // --- OFFICES ---
        const [officeRows] = await orig.query('SELECT id, name, representative_name, logo_url, is_active FROM offices');
        ...
        for (const ins of insertedOffices) {
            const old = officeRows.find((r) => r.name === ins.name);
            if (old)
                officeMap.set(old.id, ins.id);
        }
        ...
        const insUsers = insertedUsers;
        for (let i = 0; i < userOldIds.length; i++) {
            const inserted = insUsers.find((u) => u.email === userBatch[i][1]);
            if (inserted)
                userMap.set(userOldIds[i], inserted.id);
        }
        ...
        // --- CLIENTS (batch) ---
        startPing(orig);
        const [clientRows] = await orig.query(`SELECT c.id, c.full_name, c.identification, c.company_id, c.email, c.address, c.is_active,
       ...
       FROM clients c`);
        ...
        while (clientIdx < clientBatch.length) {
            ...
            const [result] = await dest.execute(`INSERT INTO clients (${clientCols.join(',')}) VALUES ${placeholders}`, flat);
        ...
        // --- CLIENT EMPLOYERS ---
        startPing(orig);
        const [empPairs] = await orig.query(`SELECT client_id, company_id, office_id,
       ...
       GROUP BY client_id, company_id, office_id`);
        ...
        for (let i = 0; i < empBatch.length; i += BATCH) {
            ...
            const flat = chunk.flat();
            const [result] = await dest.execute(`INSERT INTO client_employers (${empCols.join(',')}) VALUES ${placeholders}`, flat);
            const insertId = result.insertId;
        ...
        // --- AFFILIATIONS ---
        startPing(orig);
        const [affGroups] = await orig.query(`SELECT client_id, company_id, office_id,
       ...
       GROUP BY client_id, company_id, office_id`);
        ...
        for (let i = 0; i < affBatch.length; i += BATCH) {
            ...
            const flat = chunk.flat();
            const [result] = await dest.execute(`INSERT INTO affiliations (${affCols.join(',')}) VALUES ${placeholders}`, flat);
            const insertId = result.insertId;
        ...
        // --- MONTHLY PAYMENTS ---
        startPing(orig);
        const [monthlyRows] = await orig.query(`SELECT ma.client_id, ma.company_id, ma.month, ma.year, ma.value, ma.paid_status,
       ...
       LEFT JOIN payment_methods pm ON pm.id = ma.payment_method_id`);
        ...
        for (let i = 0; i < pmtBatch.length; i += BATCH) {
            ...
            const flat = chunk.flat();
            await dest.execute(`INSERT IGNORE INTO monthly_payments (${pmtCols.join(',')}) VALUES ${placeholders}`, flat);
        }
        log(`monthly_payments: ${monthlyCount} registros migrados`);
        // --- FACTURAS ---
        const [facturaRows] = await orig.query(`SELECT f.numero_factura, f.fecha_emision, f.valor_total, f.invoice_status, f.pdf_path
       FROM facturas f`);
